...
import placesCNN, googlenet, finetune
...
def main():
	...
	if sys.argv[1] == "snaps":
		...
		with open('../snaps/out.txt','w') as file:
			for idx, googlenet_label in enumerate(googlenet_labels):
		...
		format_output.output_labels(rel_clip_path + output_filename, output_filename, all_timestamps, image_files, shot_boundaries, classifier_label_list,
		 finetune_output, finetune_labels, googlenet_cat, googlenet_labels, scene_type_list, places_labels, scene_attributes_list)
		shutil.rmtree(clip_dir)
