...
def predict_testmode(mysvm, test_data, test_labels, orig_test_labels):
	...
	for i in range(len(output)):
		if output[i] == 'Studio':
			...
			if df_test_labels[i][0] == output[i]:
				crt_s += 1
		elif output[i] == 'Newsperson(s)':
			p_news += 1
			if df_test_labels[i][0] == output[i]:
				crt_news += 1
			elif orig_test_labels[i] == 'Background_roll':
				news_bg += 1
			elif orig_test_labels[i] == 'Graphic':
				news_g += 1
			elif orig_test_labels[i] == 'Weather':
				news_w += 1
			elif orig_test_labels[i] == 'Sports':
				news_sp += 1
		...
		if df_test_labels[i][0] == 'Studio':
			s += 1
			# print 's ', s
		elif df_test_labels[i][0] == 'Newsperson(s)':
			news += 1
	...
	print "Predicted: " , p_s , p_news, p_g, p_w, p_sp, p_bg, p_c, p_prob, p_v, p_notv, p_cr, p_notcr, p_n
	print "Correct: " , crt_s, crt_news, crt_g, crt_w, crt_sp, crt_bg, crt_c, crt_prob, crt_v, crt_notv, crt_cr, crt_notcr, crt_not
	print "Total: ", s, news, g, w, sp, bg, c, prob, v, notv, cr, notcr, n
	
	print "bg, g, w, sp", news_bg, news_g, news_w, news_sp
